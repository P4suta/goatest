...
completed event is written once and is immediately readable. Normal shutdown
syncs the stream once. A run that hangs, is interrupted, or is killed leaves a
readable prefix; only the `run-end` event is missing. An operating-system or
storage failure may lose this diagnostic trace and cannot change a verdict.
...
| `prepare` | `prepare` | one mutation-session preparation stage starts or finishes |
...
| `probe-exec` | `probe` | one infection probe or prepared exact original preflight returned |
...
`baseline`, `graph`, `race`, `probe`, `mutation`, `repair`,
...
would use. A full run enters `baseline` once for structural checks and again for
target and package controls; preparation spans can overlap the first entry.

### `prepare`

One mutation-session preparation stage. Preparation overlaps the initial
baseline structural checks, so these spans are the authoritative preparation
timeline. The mutation engine emits
a started event before the stage and a finished event after it, in deterministic
stage order.
...
A started record carries neither a result nor a duration. A finished record
carries both. A skipped stage is still a started/finished pair with a zero
duration, so a reader can distinguish an intentional omission from an event
that was lost. A failed stage is the last completed preparation stage because
the preparation call returns that failure.
...
`column`, `fallback`, `file_candidates`, `discharged`,
`probe_reaching`, `suite_coverage`, `suite_reached`, `suite_probe`, `probed`,
and `reused` are omitted when empty.
...
Every route records `granularity`. An absent `file_candidates` means zero
candidates. The schema and the trace reader reject a route without the field.
...
without it. A mutant the engine has no probe form for carries no `probed`: an
absent marker is an absent measurement rather than a mutant nothing infected,
and nothing is discharged by it.
...
Two labels are worth reading carefully. A target admitted conservatively by a
file match is not itself a route fallback. A file no test binary was ever
...
A plan entry is `individual:<target>` when a compatible execution group has one
...
infection probes; a `control` execution is the exact semantic original checked
before mutation execution and establishes no routing fact.

| Field | Meaning |
| --- | --- |
| `target` | the target that ran, `package-suite:<import-path>` for a whole-suite probe, or `mutation-control:<package>` for an exact original preflight |
| `package` | the package the target or suite belongs to |
| `suite` | `true` for a whole-package control; absent for a top-level target |
| `control` | `true` only when this is an exact semantic original mutation preflight |
...
request apart from the tree and mutant activation. Its first data-derived
budget is the sum of the passing baseline durations for targets
...
Which controls. In a full run, the exact original preflight for each distinct
mutation request reuses the session's already-compiled probe binaries with no
mutant active. The package, arguments, and environment are exactly those of
the mutant request. The preflight runs under the configured containment
ceiling, and its positive duration contributes once to the mutant budget. The
tree is the semantics-preserving probe form of the original program. The
control calibrates the sole mutant execution; it is not a post-kill
confirmation and does not cause mutant re-execution. The synthetic identity is
`mutation-control:<package>`, or `mutation-control:all` for an all-package
...
| Field | Meaning |
| --- | --- |
| `kind` | the progress kind, such as `cache-hit` or `baseline-progress` |
...
`baseline-progress`, `probe-progress`, and `mutation-progress` carry
`completed/total`. Baseline progress begins at zero; all three retain the
first, final, and evenly spaced milestones, so stream size stays bounded
independently of repository size.
...
`elapsed_ms`, and the `duration_ms` of a phase, preparation stage, command,
mutant execution, or probe execution. JSON field order is the declaration order
of the event, not a map iteration, so two recordings of the same events differ
only where time differs.
...
- each completed line is immediately readable, so a killed run leaves a
  readable prefix, while normal shutdown syncs the stream once.
...
verdict and `run-end` changes, per-event-type count deltas, outer phase-duration
deltas, and preparation stage-duration deltas. It is a read-only diagnostic
