...
2. **Project only at changed execution boundaries.** Before the first project
...
3. **Promote only the preparation delta.** The initial projection remembers its
...
4. **Keep the projection on the base filesystem.** Hard links cannot cross a
...
5. **Create quiescence when collection or refresh is due.** Every native
...
6. **Fall back, never fail verification.** An unavailable filesystem or failed
   ...
   unchanged.
7. **Bootstrap owned misses without adopting host storage.** The external
