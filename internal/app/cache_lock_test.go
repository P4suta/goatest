...
const (
	cacheLockTestDeadline  = 5 * time.Second
	cacheWaitCancellation  = 250 * time.Millisecond
	cacheReacquireDeadline = 3 * time.Second
)
...
func TestVerificationCacheWaitIsVisibleAndContextCancellationStopsBeforeRunner(t *testing.T) {
	...
	first := app.Service{Root: root, Run: func(context.Context, assure.Options) (report.Report, error) {
		close(entered)
		<-release
		return report.Report{Schema: report.SchemaV1, Verdict: report.VerdictAssured, Contract: "standard-v1", Snapshot: appTestDigest("a")}, nil
	}}
	...
	select {
	case <-entered:
	case <-time.After(cacheLockTestDeadline):
	...
	ctx, cancel := context.WithTimeout(context.Background(), cacheWaitCancellation)
	defer cancel()
...
func TestARunCollectsExpiredRecordingsUnderTheLeaseItOwns(t *testing.T) {
	...
	if err := os.MkdirAll(stale, filemode.ReadableDirectory); err != nil {
		t.Fatal(err)
	}
	...
	ctx, cancel := context.WithTimeout(t.Context(), cacheReacquireDeadline)
	defer cancel()
