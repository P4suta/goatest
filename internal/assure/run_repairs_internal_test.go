...
func TestRunCoordinatorClosesRoundOnPrepareMutationGenerationAndCloseFailures(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			wantWorkspaces, wantGraph := completedRoundWorkspaceCount, completedRoundGraphPersistCount
			if test.name == "prepare" {
				wantWorkspaces, wantGraph = preparedFailureWorkspaceCount, 0
			}
			if !errors.Is(err, cause) || !reflect.DeepEqual(result, report.Report{}) || harness.manager.calls != 1 || harness.workspaceCloses != wantWorkspaces ||
				harness.buildGraphCalls != wantGraph || harness.mergeGraphCalls != wantGraph || harness.saveGraphCalls != wantGraph {
...
func TestRunCoordinatorReopensFreshSnapshotAfterGeneratedRepair(t *testing.T) {
	...
	if err != nil || result.Verdict != report.VerdictAssured || !reflect.DeepEqual(result.Repairs, []report.Repair{repair}) ||
		harness.openCalls != completedRepairRoundCount*completedRoundWorkspaceCount ||
		harness.generationCalls != completedRepairRoundCount ||
		harness.workspaceCloses != completedRepairRoundCount*completedRoundWorkspaceCount {
		t.Fatalf("run = (%+v, %v), harness=%+v", result, err, harness)
	}
}
...
func TestRunCoordinatorStopsAtThreeRepairRoundsWithExplicitFinding(t *testing.T) {
	...
	harness.dependencies.attemptRepairs = func(context.Context, string, []report.Finding, GenerationOptions) (GenerationEvaluation, error) {
		harness.generationCalls++
		return GenerationEvaluation{Findings: []report.Finding{finding}, Repairs: []report.Repair{{ID: "repair-" + string(rune('0'+harness.generationCalls)), Status: "applied"}}, Applied: true}, nil
	}
	result, err := harness.run(Options{})
	if err != nil || result.Verdict != report.VerdictInsufficient || len(result.Repairs) != maximumRounds || len(result.Findings) != 2 ||
		result.Findings[1].Kind != "repair-round-limit" || harness.openCalls != maximumRounds*completedRoundWorkspaceCount ||
		harness.workspaceCloses != maximumRounds*completedRoundWorkspaceCount || harness.manager.calls != maximumRounds ||
		harness.generationCalls != maximumRounds || len(harness.cache.puts) != 0 || harness.buildGraphCalls != maximumRounds ||
...
func TestRunCoordinatorRejectsFinalInputErrorsAndRepositoryDrift(t *testing.T) {
	...
	t.Run("scan error", func(t *testing.T) {
		...
		harness.dependencies.assuranceInputs = func(string, string, Options, config.Config, roundMetadata) (evidence.Inputs, string, error) {
			harness.inputCalls++
			if harness.inputCalls == finalInputScanCount {
				return evidence.Inputs{}, "", cause
			}
			return harness.inputs, harness.digest, nil
		}
		result, err := harness.run(Options{})
		if !errors.Is(err, cause) || !reflect.DeepEqual(result, report.Report{}) || harness.workspaceCloses != completedRoundWorkspaceCount || harness.inputCalls != finalInputScanCount ||
